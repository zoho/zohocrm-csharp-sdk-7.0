...
namespace Samples.UserGroups
{
	...
	{
		public static void GetGroupedCounts_1(long groupId)
		{
			...
			if (response != null)
			{
				...
				if (response.IsExpected)
				{
					...
					if (responseHandler is GroupedCountWrapper)
					{
						GroupedCountWrapper responseWrapper = (GroupedCountWrapper) responseHandler;
						List<GroupedCount> groupedCounts = responseWrapper.GroupedCounts;
						foreach (GroupedCount groupedCount in groupedCounts)
						{
                            Console.WriteLine("User Status: " + groupedCount.Status.Value);
                            Console.WriteLine("User Count: " + groupedCount.Count);
						}
